...
namespace eds.sorteremulator.communiation.Sockets
{
    public class TCPSocket
    {
        ...
        private readonly ILogger<TCPSocket> _logger;
        ...
        public bool IsConnected
        {
            get
            {
                ...
                {
                    _logger.LogError("Exception in IsConnected", ex);
                    return false;
                }
            }
        }
        ...
        public TCPSocket(ILogger<TCPSocket> logger)
        {
            ConnectionPort = -1;
        ...
        public void Start()
        {
            try
            {
                ...
                if (_checkConnectionThread.IsAlive)
                    return;
                _logger.LogWarning("Timeout to create check connection thread. Socket will be stopped");
                Stop();
            }
            catch (Exception ex)
            {
                _logger.LogError("Exception in socket start. Socket will be stopped", ex);
                Stop();
            }
        }
        ...
        public void Stop()
        {
            ...
            {
                _logger.LogError("Error en viMessageSocket.Stop() " + ex.Message, ex);
            }
        }
           ...
           while (!_markToDelete)
            {
                ...
                {
                    _logger.LogError("Exception in Check Connection Thread", ex);
                }
            }
            if (!_requestStopped)
                _logger.LogWarning("Check Connection Thread was stopped without request ");
        ...
        private void Connect()
        {
            ...
            if (!IsServer)
            {
                ...
                {
                    _logger.LogError("Exception when connecting in client mode", ex);
                    Disconnect();
                }
            }
            else
            {
                try
                {
                    ...
                    if (_socket != null && _socket.IsBound)
                    {
                        ...
                        {
                            _socket = null;
                            _logger.LogError("Exception when listenting server mode", e);
                        }
                    }
                    else
                    {
                ...
                {
                    _logger.LogError("Exception when connecting in server mode", ex);
                    Disconnect();
                }
            }
        }
        ...
        private void CreateServerSocket()
        {
            ...
            {
                _logger.LogError("Error when creating server socket", e);
            }
        }
        ...
        public void DisconnectCallback(IAsyncResult ar)
        {
            ...
            {
                _logger.LogError("Error processing DisconnectCallback", e);
            }
        }
        ...
        private void CheckActivity()
        {
            ...
            if (!OnActivityTimeOut(dt.TotalMilliseconds))
                return;
            _logger.LogWarning("Disconnected for Inactivity! (" + dt.TotalMilliseconds + ")");
        ...
        private void ConnectToServerCallback(IAsyncResult ar)
        {
            try
            {
                ...
                _logger.LogInformation($"Socket {SocketName} connected to {Socket.RemoteEndPoint} trough {Socket.LocalEndPoint}");
            }
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                {
                    ...
                    {
                        _logger.LogWarning($"Connection Refused when trying to connect: {e.Message}");
                ...
                else
                {
                    if (_lastErrorServerLogged.AddSeconds(60) < DateTime.UtcNow)
                    {
                        _logger.LogCritical("Exception while trying to connect", e);
                        _lastErrorServerLogged = DateTime.UtcNow;
                    }
                    //Descanso
                    Thread.Sleep(500);
            ...
            {
                _logger.LogError("Exception while trying to connect", ex);
                Disconnect();
            }
            _mEventsSocket.Set();
        }
        ...
        private void ConnectFromClientCallback(IAsyncResult ar)
        {
            try
            {
                ...
                OnSocketStateChange(SocketState.Connected);
                _firstConnection = false;
                _logger.LogInformation($"Socket {SocketName} received connection from {Socket.RemoteEndPoint} trough {_socket.LocalEndPoint}");
                handler.BeginReceive(state.buffer, 0, StateObject.BufferSize, 0, ReadCallback, state);
            }
            catch (Exception ex)
            {
                _logger.LogError("Exception while accepting client connection", ex);
                Disconnect();
            }
        }
        ...
        public void ProcessDataReceived(byte[] readBuffer, int bytesReaded)
        {
            ...
            while (!completed) //Bucle para procesar mientras nos quede información susceptile de ser o formar parte de un message
            {
                ...
                if (!_foundStartMarker) //No teniamos ningún message previamente iniciado, por lo que en esta parte buscamos abrir uno nuevo
                {
                    ...
                    //if (idx > 0) _logger.LogWarning("Discarded content: " + received.Substring(0, idx + 1));
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(received))
                        {
                            ...
                            if (stringBuilder.Length > MaxMessageLength + EndMarker.Length)
                            {
                                ...
                                _logger.LogWarning("Message lenght exceds the buffer size. Restart socket");
                            }
                        }
                        completed = true;
                    }
        ...
        private void OnMessageReceived(string message, Stopwatch sw)
        {
            ...
            {
                _logger.LogWarning($"Discard Message {message} takes {elapsed.ToString("00000")} ms to start the task");
                return;
            }
            LastActivity = DateTime.UtcNow;
            Timeout = DateTime.UtcNow;
        ...
        public bool Send(string data)
        {
            ...
            if (IsConnected)
            {
                ...
                {
                    _logger.LogError("There is a problem sendig the message",ex);
                    salida = false;
                }
            }
            return salida;
        ...
        private void SendCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (bytesSent == 0)
                {
                    _logger.LogWarning("0 bytes sent but the message has " + state.buffer.Length + " bytes ");
                }
            ...
            {
                _logger.LogError("Error enviando message " + (state != null && state.buffer != null && state.buffer.Count() > 0 ? Encoding.ASCII.GetString(state.buffer) : "'SIN DATOS?'") + ex.Message, ex);
            }
        }
        ...
        private void ReadCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogCritical("Fatal Exception on data reception", ex);
                    Disconnect();
                }
            }
            catch (ObjectDisposedException)
            ...
            {
                _logger.LogCritical("Fatal Exception on data reception. The socket will be disconnected ", ex);
                Disconnect();
            }
        }
